...
<body>
  ...
  <div class="filter-group">
    <p>Availability</p>
    <label><input type="checkbox" checked /> In stock (<c:out value="${fn:length(shrugs)}" />)</label>
    ...
    <section class="products-section">
      ...
      <div class="page-title">
        <div class="title-group">
            <h2>${search }</h2>
            <p><a href="${pageContext.request.contextPath}/">Home </a>> SearchResult</p>
        </div>
        <div class="sort">
...
<div class="product-grid">
  <c:forEach var="product" items="${products}">
    ...
    <button type="submit" class="icon" style="border:none; cursor:pointer;">
      <i class='bx bx-heart'></i>
        ...
        <form action="${pageContext.request.contextPath}/searchController/product" method="get">
  ...
  <c:if test="${currentPage > 1}">
    <a href="${pageContext.request.contextPath}/kurtisController/kurtis?page=${currentPage - 1}">&laquo;</a>
  </c:if>

  <c:forEach var="i" begin="1" end="${totalPages}">
    <c:choose>
      <c:when test="${i == currentPage}">
        <a class="active" href="${pageContext.request.contextPath}/kurtisController/kurtis?page=${i}">${i}</a>
      </c:when>
      <c:otherwise>
        <a href="${pageContext.request.contextPath}/kurtisController/kurtis?page=${i}">${i}</a>
      </c:otherwise>
    </c:choose>
  </c:forEach>

  <c:if test="${currentPage < totalPages}">
    <a href="${pageContext.request.contextPath}/kurtisController/kurtis?page=${currentPage + 1}">&raquo;</a>
  </c:if>
</div>
